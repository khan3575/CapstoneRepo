"""
Train a single cross-validation fold  — v5: BCE + DiceLoss + pos_weight + flip augmentation
================================================
Builds on v4 (BCE+DiceLoss). Two additions:
  1. pos_weight=16 in BCE — tumor nodes are ~5.8% of all nodes (16x minority).
     Tells BCE: a missed tumor node costs 16x more than a missed background node.
  2. RandomFlipTransform — randomly mirrors norm_x (feature index 11) with p=0.5.
     Applied to TRAIN only. Val/test use BinaryTransform only (no augmentation).
Saves to checkpoints/binary_v5/ — does NOT touch binary_v3 or binary_v4.
See FIX_LOG.md -> DECISION-1 and NEXT_STEPS.md -> Branch A for context.
"""
...
class RandomFlipTransform:
    """
    v5 augmentation: random horizontal flip applied to TRAIN graphs only.
    Feature layout (15 features):
      0-3:  T1/T1ce/T2/FLAIR means
      4-7:  T1/T1ce/T2/FLAIR stds
      8:    area        9: norm_area
      10:   norm_y      11: norm_x   ← flip target
      12:   perimeter   13: compactness   14: intensity_range
    Flip: norm_x -> (1 - norm_x). Mirrors centroid left/right.
    Intensity and shape features are flip-invariant — no change needed.
    Also applies BinaryTransform (label binarisation) so it replaces it for train.
    """
    def __init__(self, p=0.5):
        self.p = p
        self._binary = BinaryTransform()

    def __call__(self, data):
        data = self._binary(data)
        if _random.random() < self.p:
            data.x = data.x.clone()
            data.x[:, 11] = 1.0 - data.x[:, 11]  # mirror norm_x
        return data
...
def train_fold(
    ...
    # v5 transforms:
    # - Train: RandomFlipTransform (includes BinaryTransform + p=0.5 horizontal flip)
    # - Val/Test: BinaryTransform only (no augmentation — deterministic evaluation)
    train_transform = RandomFlipTransform(p=0.5)
    eval_transform  = BinaryTransform()
    ...
    train_dataset = BraTSGraphDataset(
        ...
        transform=train_transform  # ← binary + random flip
    )
    ...
    val_dataset = BraTSGraphDataset(
        ...
        transform=eval_transform  # ← binary only, no augmentation
    )
    ...
    # Loss and optimizer — v5: BCE(pos_weight) + DiceLoss (equal weight 0.5 each)
    # pos_weight: passed via CLI (--pos_weight). Default=16. Search script tests 4/6/8/10/12.
    # Tells BCE that a missed tumor node costs pos_weight x more than a missed background node.
    # DiceLoss: directly optimises the Dice metric; handles class imbalance structurally.
    # CrossSliceConsistencyLoss intentionally excluded — model has no cross-slice architecture.
    print(f"  Loss: BCE(pos_weight={pos_weight}) + DiceLoss (0.5 + 0.5)")
    _pos_weight = torch.tensor([float(pos_weight)]).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pos_weight', type=float, default=16.0,
                       help='BCE pos_weight for tumor class (default: 16). Search script tests 4/6/8/10/12.')
