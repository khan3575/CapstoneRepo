...
\textbf{1. Pure GNN Architecture:} We demonstrate that a lightweight GraphSAGE model (439K parameters) without any convolutional components can achieve 91.41\% Dice—approaching state-of-the-art transformers while offering 155$\times$ parameter reduction and 6.9$\times$ inference speedup.
...
\textbf{3. Clinical Deployment Focus:} By achieving near-real-time performance (74\,ms per patient with pre-built graph; 1.47\,s end-to-end) on consumer-grade hardware (RTX 2060 6GB), we demonstrate practical viability for resource-constrained healthcare settings.
