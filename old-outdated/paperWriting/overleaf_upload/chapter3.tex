...
\textbf{Phase 4: Model Training} employs a 5-layer GraphSAGE architecture with 256 hidden dimensions. We train using AdamW optimizer (learning rate 0.001), binary cross-entropy loss with class weights to handle imbalance, effective batch size 64 (implemented as batch size 32 with 2 gradient accumulation steps), and 50 epochs. Training uses deterministic seed (42) and CUDA deterministic mode for reproducibility.
...
\textbf{Phase 6: Ensemble Methods} combine predictions from all 5 folds using soft voting (averaging sigmoid outputs) to leverage model diversity and improve robustness. The ensemble yields a 1.39\% absolute improvement in Dice over the cross-validation mean (90.02\% $\rightarrow$ 91.41\% on the sealed 251-patient held-out set).
...
\textbf{Rationale:} Each fold trains on different patient subsets, learning complementary patterns. Averaging reduces prediction variance and improves generalisation. Evaluated on the sealed 251-patient held-out set, soft-voting yields a 1.39\% Dice improvement over the cross-validation mean (90.02\% $\rightarrow$ 91.41\%).
