...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_A_cv_performance.png}
...
The $\pm$1 SD band is shaded grey. The $+1.39\%$ ensemble lift is annotated in the right margin.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig_E_ensemble_vs_individual.png}
...
The ensemble achieves $+1.39\%$ absolute Dice improvement (90.02\%\,$\rightarrow$\,91.41\%), confirmed as
statistically significant by one-sample $t$-test ($t=-4.19$, $p=0.014$, 95\% CI $[89.10\%,90.94\%]$, $df=4$).
...
We compare against a standard 3D U-Net baseline (68M parameters) implemented with identical evaluation protocols under the same hardware conditions. This baseline represents a standard volumetric CNN architecture. For reference, published SOTA models occupy a similar parameter scale: Swin-UNETR (62M), nnU-Net (\textasciitilde31M for the BraTS 3D full-resolution configuration). We select this 68M U-Net variant because it is our directly implemented and hardware-benchmarked reference, trained under identical RTX 2060 / 6\,GB VRAM conditions, allowing a fair and reproducible efficiency comparison free from confounds introduced by architectural innovations (transformers, automatic hyperparameter search) or differing hardware.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_D_speed_memory.png}
\caption{\textbf{Computational Efficiency: GNN vs.\ 3D U-Net Baseline.}
All measurements on NVIDIA RTX 2060, 6\,GB VRAM.
\textit{Left:} End-to-end inference time per patient. GNN end-to-end (1.47\,s including SLIC) is 6.9$\times$ faster than U-Net (10.16\,s); GNN with pre-built graph (74\,ms) is $>$137$\times$ faster.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig_B_efficiency_bubble.png}
\caption{\textbf{Accuracy vs.\ Efficiency Trade-off.}
Each point represents a model; bubble area is proportional to $\sqrt{\text{parameters}}$ for readability.
The GNN single model (439K, 1.47\,s, 90.02\% Dice) and ensemble (2.2M, 1.47\,s, 91.41\%) both appear in the upper-left (fast and accurate) compared to the 3D U-Net (68M, 10.16\,s, 87.5\%).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig_F_ablation.png}
...
The baseline GraphSAGE 5-layer 256-dim (439K) achieves 84.03\%, representing the Pareto-optimal
accuracy-efficiency operating point: the wider 512-dim variant gains $+4.75\%$ at $3.9\times$ the parameter cost,
the deeper 6-layer variant offers no benefit ($-0.03\%$), and GAT gains $+1.00\%$ at $2.7\times$ cost with slower training.}
\label{fig:ablation}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{failure_cases/worst_BraTS2021_01405_z108.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{failure_cases/median_BraTS2021_00209_z059.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{failure_cases/best_BraTS2021_01594_z097.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_G_dice_distribution.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig_C_generalisation.png}
\caption{\textbf{Cross-Dataset Generalisation: BraTS 2021 Held-Out vs.\ BraTS 2023 Zero-Shot.}
...
\textit{Right:} Per-metric generalisation gap (BraTS 2021 minus BraTS 2023). Red bars indicate performance drop; green bars indicate improvement. The dominant gap is in Dice ($-2.20$\,pp); Sensitivity improves by $+2.29$\,pp on BraTS 2023, indicating the model retains tumour recall across datasets. The inset box summarises the overall Dice gap: 91.41\%\,$\rightarrow$\,89.21\%.}
...
\hline
nnU-Net (2021) \cite{isensee2021nnunet} & 92.5 & 31M & ET+TC+WT & Automatic configuration\\
\hline
...
\end{tabularx}
\end{center}
{\footnotesize \textit{Context note:} Multi-class methods benefit from auxiliary gradient signal across all three sub-region heads during training; our binary model has no such auxiliary supervision. The 1.1--1.9\% gap between our ensemble (91.41\%) and the leading BraTS 2021 methods (nnU-Net 92.5\%; Swin-UNETR 93.3\%) should be interpreted in light of this structural disadvantage.
