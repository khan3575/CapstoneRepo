...
This chapter lays out the methodology we used to develop and validate the graph-based neural network for brain tumor segmentation. The research follows a systematic pipeline—data preprocessing, graph construction, model architecture design, training, and rigorous validation. Everything's designed to ensure reproducibility, eliminate data leakage, and enable fair comparison with baseline methods.

Our framework converts 3D multi-parametric MRI volumes into 2D graph representations. Superpixels become nodes, and spatial adjacency defines edges. This cuts computational complexity while preserving essential tumor characteristics through carefully engineered node features. The GNN processes these graphs using message-passing layers that aggregate neighborhood information, enabling both  local texture analysis and global structural reasoning. All experimental procedures follow deterministic protocols with comprehensive integrity checks to ensure scientific rigor and clinical reliability.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{image/pipeline_architechture.png}
\caption{Complete 8-Phase Pipeline Architecture.}
\label{fig:framework}
\end{figure}
...
\textbf{Phase 5: Cross-Validation} implements patient-level stratified 5-fold splits—no patient appears in both training and validation sets. Each fold trains independently with identical hyperparameters, which enables robust performance estimation and statistical significance testing.

\textbf{Phase 6: Ensemble Methods} combine predictions from all 5 folds using soft voting (averaging sigmoid outputs) to leverage model diversity and improve robustness. The ensemble consistently shows 2-3\% improvement over individual models.
...
\textbf{1. Multi-Modal Intensity Statistics (12 features):} For each of the four MRI modalities (T1, T1CE, T2, FLAIR), we compute statistical descriptors over all voxels within each superpixel:
\begin{itemize}
    \item Mean intensity: captures average signal
    \item Standard deviation: measures local intensity variation
    \item Minimum/Maximum intensities: capture intensity range
    \item Median intensity: robust central tendency
\end{itemize}

Why these? Glioma tumors exhibit characteristic intensity patterns: hypo-intense in T1, hyper-intense in T2/FLAIR, rim enhancement in T1CE. These statistics let the GNN learn discriminative tumor signatures.

\textbf{2. Spatial Features (2 features):} Normalized (x, y) centroid coordinates of each superpixel provide spatial context, enabling the model to learn anatomical priors about where tumors typically occur.

\textbf{3. Geometric/Shape Features (1 feature):} Superpixel area captures region size, helping distinguish focal lesions from larger tumor masses.

\textbf{Critical Design Decision:} What's notably absent from our features is tumor label information or tumor ratio within superpixels. Initial experiments included a "tumor\_ratio" feature (percentage of tumor pixels in each superpixel), achieving 98\% accuracy—but that's severe data leakage. After removing this feature and retraining, we got the validated 90.39\% performance. That ensures fair evaluation and generalization capability.
...
\textbf{Step 2: Superpixel Generation} - Apply SLIC (Simple Linear Iterative Clustering) algorithm to each slice with parameters: n\_segments=50-100 (target number per slice), compactness=10 (balance between spatial and intensity clustering). SLIC groups similar pixels into perceptually coherent regions that often align with anatomical structures.

\textbf{Node Count Design Choice:} We configure SLIC to generate approximately \textbf{80-100 superpixels per slice} (parameter experimentation determined this optimal range), resulting in roughly \textbf{10,000 total nodes per patient} across all 200 slices. This design balances computational efficiency with spatial resolution—denser superpixel sampling (e.g., 800 nodes/slice yielding 160K total nodes) would preserve finer texture details but increase graph processing cost by 16$\times$, while sparser sampling (e.g., 20 nodes/slice yielding 4K total nodes) would oversimplify tumor boundaries. Preliminary experiments confirmed that 80-100 nodes/slice provides the optimal accuracy-efficiency trade-off for brain tumor segmentation.
...
\textbf{Computational Efficiency:} Converting a 3D volume (240$\times$240$\times$155 = 8.9M voxels) into a graph (200 slices $\times$ $\sim$50 superpixels average = $\sim$10K nodes) reduces representation size by \textbf{890$\times$}, enabling efficient GNN processing while preserving tumor boundary information through explicit edge connectivity.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{image/graphsage_architecture.png}
\caption{5-Layer GraphSAGE Architecture.}
...
\textbf{Loss Function:} Binary Cross-Entropy (BCE) loss with class weights to address extreme class imbalance (tumor nodes $\sim$10\%, non-tumor $\sim$90\%):

\begin{equation}
\mathcal{L} = -\frac{1}{N} \sum_{i=1}^{N} \left[ w_1 \cdot y_i \log(\hat{y}_i) + w_0 \cdot (1-y_i) \log(1-\hat{y}_i) \right]
\end{equation}

where $w_1 = 9.0$ (weight for tumor class) and $w_0 = 1.0$ (weight for non-tumor class).

\textbf{Optimization:} Adam optimizer with learning rate 0.001, $\beta_1=0.9$, $\beta_2=0.999$, weight decay $10^{-5}$.

\textbf{Batch Configuration:} Batch size 32 (optimal discovered through empirical analysis), gradient accumulation disabled. Larger batches (48, 64) showed performance degradation.

\textbf{Training Duration:} 100 epochs with early stopping (patience=15 epochs) based on validation Dice coefficient. Typical convergence occurs around epoch 50-70.
...
The complete pipeline is implemented in Python using PyTorch and PyTorch Geometric frameworks. Medical image processing utilizes SimpleITK and nibabel for NIfTI file handling, while graph construction employs scikit-image's SLIC algorithm for superpixel generation. Preprocessed data is stored as compressed .npz files (4 modalities $\times$ 200 slices) for efficient loading during training. Graph representations are structured as PyTorch Geometric Data objects containing node features (N $\times$ 15), edge indices (2 $\times$ E), and labels (N $\times$ 1).

Several implementation choices warrant emphasis for reproducibility: (1) \textbf{deterministic training} with manual seed 42 across Python, NumPy, PyTorch, and CUDA environments using \texttt{CUBLAS\_WORKSPACE\_CONFIG} and \texttt{CUDNN\_DETERMINISTIC} flags, (2) \textbf{batch size optimization} discovering that batch 32 significantly outperforms larger batches (48, 64) in medical imaging GNN tasks, (3) \textbf{comprehensive integrity auditing} with 15 automated validation checks detecting data leakage, patient overlap, and normalization errors, (4) \textbf{efficient data structures} converting 8.9M voxel volumes into 10K node graphs (890$\times$ compression) while preserving tumor-discriminative features, and (5) \textbf{ensemble post-processing} using soft voting to aggregate sigmoid probabilities from 5 folds before thresholding at 0.5. All experiments run on consumer-grade hardware (NVIDIA RTX 2060, 6GB VRAM) demonstrating accessibility. The complete codebase with documentation is available for reproduction, enabling other researchers to validate our findings and build upon this work.

\vspace{5mm}

This methodology chapter has presented a comprehensive and rigorous approach to graph-based brain tumor segmentation. The proposed framework systematically transforms 3D MRI volumes into graph representations through superpixel-based node generation and careful feature engineering that avoids ground-truth leakage. The GraphSAGE architecture with 5 layers and 256 hidden dimensions provides an optimal balance between expressiveness and parameter efficiency, validated through systematic ablation studies. Patient-level stratified cross-validation ensures statistically valid performance estimation, while the ensemble strategy leverages model diversity for robust predictions. Comprehensive integrity validation with 15 auditing checks (covering data isolation, feature validity, graph structure, reproducibility, and output verification) guarantees reproducibility and eliminates data leakage, addressing critical concerns in medical AI research. The modular pipeline design enables efficient experimentation, adaptation to new datasets, and potential clinical translation. The subsequent chapter will present experimental results demonstrating that this methodology achieves competitive accuracy (92.92\% Dice) with substantial efficiency improvements (6.9$\times$ speedup, 156$\times$ parameter reduction) suitable for real-world clinical deployment.
